-- Fix the cascade deletion issue by updating the delete_user_account_secure function
-- to mark bookings as cancelled instead of deleting them when facilities are deleted
...
BEGIN
  ...
  -- CRITICAL: Mark ALL bookings on facilities owned by this user as cancelled 
  -- This includes past and future bookings so clients can see their history
  ...
  WHERE facility_id IN (
    SELECT id FROM facilities WHERE owner_id = _user_id
  );
  ...
  -- Now safely delete user facilities (bookings are preserved but marked as cancelled)
