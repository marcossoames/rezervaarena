-- Security fixes migration (refined)
...
-- 2. Add trigger to ensure profile creation on user signup (if not exists)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 3. Tighten facilities RLS - Remove client direct access policy
...
-- 4. Fix storage policies for facility-images bucket
...
-- Ensure public read policy exists for client viewing
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Facility images are publicly viewable'
  ) THEN
...
-- 5. Add server-side data validation constraints (if not exists)
DO $$
BEGIN
  IF NOT EXISTS (
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'capacity_must_be_positive'
  ) THEN
    ALTER TABLE public.facilities 
...
-- 6. Add booking overlap prevention function
...
-- Add trigger for booking overlap checking (if not exists)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'check_booking_overlap_trigger'
  ) THEN
