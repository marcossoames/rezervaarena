-- Update delete_user_account_secure to send cancellation emails for active bookings
DROP FUNCTION IF EXISTS public.delete_user_account_secure(uuid) CASCADE;
...
BEGIN
  ...
  IF active_booking_count > 0 THEN
    ...
    -- Send cancellation emails via edge function (asynchronous)
    -- This uses a background task pattern to avoid blocking the deletion
    PERFORM net.http_post(
      url := 'https://ukopxkymzywfpobpcana.supabase.co/functions/v1/send-booking-cancellation-email',
      ...
      body := jsonb_build_object(
        ...
        'facilityName', array_to_string(facility_names, ', '),
        'reason', 'Proprietarul facilității și-a șters contul de pe platformă.'
      )::text
    );
  END IF;
