-- Fix availability function to properly exclude cancelled bookings
...
CREATE OR REPLACE FUNCTION public.get_facility_availability_secure(facility_id_param uuid, booking_date_param date)
 RETURNS TABLE(start_time time without time zone, end_time time without time zone, unavailable_type text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  RETURN QUERY
  -- Only confirmed and pending bookings (exclude cancelled, completed, no_show)
  SELECT 
    b.start_time,
    b.end_time,
  ...
  WHERE b.facility_id = facility_id_param 
    ...
    AND b.status IN ('confirmed', 'pending') -- EXCLUDE cancelled, completed, no_show
