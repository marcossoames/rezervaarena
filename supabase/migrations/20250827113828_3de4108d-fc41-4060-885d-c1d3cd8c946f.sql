-- Fix Critical Security Issues (without duplicate storage policy)
...
DROP TRIGGER IF EXISTS trigger_validate_booking_security ON public.bookings;
DROP TRIGGER IF EXISTS trigger_prevent_booking_overlap ON public.bookings;
DROP TRIGGER IF EXISTS trigger_protect_booking_integrity ON public.bookings;
DROP TRIGGER IF EXISTS trigger_update_booking_updated_at ON public.bookings;
DROP TRIGGER IF EXISTS trigger_update_client_booking_stats ON public.bookings;
DROP TRIGGER IF EXISTS trigger_update_facility_updated_at ON public.facilities;
...
-- 4. Add secure storage policies for facility-images bucket
DROP POLICY IF EXISTS "Users can upload to their own facility images" ON storage.objects;
