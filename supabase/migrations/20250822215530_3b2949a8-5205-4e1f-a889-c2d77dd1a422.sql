-- Update the client booking function to show exact details for authenticated clients
DROP FUNCTION public.get_facilities_for_booking();
...
AS $function$
  -- Enhanced data for authenticated clients - show exact pricing and capacity for booking decisions
  SELECT 
    f.id,
    f.name,
    f.facility_type,
  ...
  WHERE f.is_active = true
    ...
    AND has_role('client'::user_role)  -- Only for clients
