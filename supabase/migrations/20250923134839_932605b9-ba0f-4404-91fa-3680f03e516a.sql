-- Add recurring blocking functionality for entire days
...
-- Function to generate blocked dates from recurring patterns
...
BEGIN
  ...
  days_to_add := (target_day - EXTRACT(DOW FROM current_date)::INTEGER + 7) % 7;
  current_date := current_date + days_to_add;
  
  -- Generate entries until end_date or 6 months, whichever comes first
  WHILE current_date <= COALESCE(NEW.end_date, current_date + INTERVAL '6 months') LOOP
