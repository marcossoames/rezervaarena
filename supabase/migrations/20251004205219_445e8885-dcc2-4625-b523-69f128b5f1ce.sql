-- Drop and recreate the v2 functions with proper error handling
DROP FUNCTION IF EXISTS public.promote_user_to_admin_v2(uuid, uuid);
DROP FUNCTION IF EXISTS public.promote_user_to_facility_owner_v2(uuid, uuid);
DROP FUNCTION IF EXISTS public.demote_admin_to_client_v2(uuid, uuid);

-- Recreate promote_user_to_admin_v2 with caller_user_id
...
-- Recreate promote_user_to_facility_owner_v2
...
-- Recreate demote_admin_to_client_v2 (ONLY for super_admin)
...
BEGIN
  ...
  -- Only super_admin or the designated email can demote admins
  SELECT EXISTS (
    SELECT 1 FROM public.user_roles 
    WHERE user_id = _caller_user_id AND role = 'super_admin'::app_role
  ) INTO caller_is_super_admin;
  
  IF NOT (caller_is_super_admin OR caller_email = 'soamespaul@gmail.com') THEN
