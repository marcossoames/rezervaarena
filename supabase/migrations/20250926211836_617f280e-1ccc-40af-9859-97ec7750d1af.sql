...
-- Create function to cancel bookings instead of deleting when account is deleted
...
BEGIN
  ...
  IF user_role = 'facility_owner' THEN
    ...
    LOOP
      ...
      -- Collect booking details for email sending
      booking_info := booking_info || jsonb_build_object(
  ...
  ELSE
    ...
    LOOP
      ...
      -- Collect booking details for facility owner notification
      booking_info := booking_info || jsonb_build_object(
        'booking_id', booking_rec.id,
...
-- Create function to delete user account with proper booking cancellation
...
BEGIN
  ...
  -- Delete user data but keep booking history
  DELETE FROM bank_details WHERE user_id = current_user_id;
