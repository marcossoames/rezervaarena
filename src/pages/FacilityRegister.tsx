...
const FacilityRegister = () => {
  ...
  const removeAmenityFromFacility = (facilityIndex: number, amenityToRemove: string) => {
    const updatedFacilities = [...facilities];
    updatedFacilities[facilityIndex].amenities = updatedFacilities[facilityIndex].amenities.filter(amenity => amenity !== amenityToRemove);
    setFacilities(updatedFacilities);
  };
  ...
  const renderStep1 = () => <form onSubmit={handleSubmit(handleStep1Submit)} className="space-y-6">
      {/* Account Information */}
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Input id="phone" type="tel" {...register("phone")} className="bg-background/50" />
          </div>
        </div>
      ...
      <div className="space-y-4">
        ...
        })} className="bg-background/50" placeholder="ex: Complexul Sportiv Arena" />
        ...
        <div className="space-y-2">
          ...
          <Textarea id="businessDescription" {...register("businessDescription")} className="bg-background/50 min-h-[100px]" placeholder="Descrieți baza dumneavoastră sportivă..." />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="numberOfFacilities">Numărul de Facilități *</Label>
            <Input id="numberOfFacilities" type="number" min="1" max="10" {...register("numberOfFacilities", {
            required: "Numărul de facilități este obligatoriu",
            valueAsNumber: true,
            min: {
              value: 1,
              message: "Trebuie să aveți cel puțin o facilitate"
            },
            max: {
              value: 10,
              message: "Maxim 10 facilități"
            }
          })} className="bg-background/50" placeholder="ex: 3" />
            {errors.numberOfFacilities && <p className="text-sm text-destructive">{errors.numberOfFacilities.message}</p>}
          </div>
        </div>
        ...
        })} className="bg-background/50" placeholder="Strada, numărul, sectorul/comuna" />
        ...
        <div className="bg-muted/30 p-4 rounded-lg border border-dashed border-muted-foreground/20">
          ...
          <p className="text-xs text-muted-foreground">Fiecare facilitate va avea propriile imagini, preț și facilități.</p>
        </div>
      </div>

      <Button type="submit" className="w-full h-12 text-lg font-semibold bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70">
  ...
  const renderStep2 = () => <div className="space-y-6">
      ...
      {facilities.map((facility, index) => <Card key={index} className="border-2">
          <CardHeader>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input value={facility.name} onChange={e => updateFacilityField(index, 'name', e.target.value)} placeholder="ex: Teren Fotbal 1, Teren Tenis A" className="bg-background/50" />
              </div>

              <div className="space-y-2">
                <Label>Tipul Facilității *</Label>
                <Select value={facility.facilityType} onValueChange={value => updateFacilityField(index, 'facilityType', value)}>
            ...
            <div className="space-y-2">
              ...
              <Textarea value={facility.description} onChange={e => updateFacilityField(index, 'description', e.target.value)} placeholder="Descrieți această facilitate specifică..." className="bg-background/50" />
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label>Preț/Oră (RON) *</Label>
                <Input type="number" min="1" step="0.01" value={facility.pricePerHour || ''} onChange={e => updateFacilityField(index, 'pricePerHour', parseFloat(e.target.value) || 0)} className="bg-background/50" />
              </div>

              <div className="space-y-2">
                <Label>Capacitate *</Label>
                <Input type="number" min="1" value={facility.capacity || ''} onChange={e => updateFacilityField(index, 'capacity', parseInt(e.target.value) || 1)} className="bg-background/50" />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Input value={amenityInputs[index] || ''} onChange={e => {
              const newInputs = [...amenityInputs];
              newInputs[index] = e.target.value;
              setAmenityInputs(newInputs);
            }} placeholder="Adaugă facilitate (ex: vestiare, duș)" className="bg-background/50" onKeyPress={e => {
              if (e.key === 'Enter') {
                e.preventDefault();
                addAmenityToFacility(index, amenityInputs[index] || '');
              }
            }} />
                <Button type="button" onClick={() => addAmenityToFacility(index, amenityInputs[index] || '')} size="icon" variant="outline">
                  <Plus className="h-4 w-4" />
                </Button>
              </div>
              {facility.amenities.length > 0 && <div className="flex flex-wrap gap-2 mt-2">
                  {facility.amenities.map(amenity => <Badge key={amenity} variant="secondary" className="flex items-center gap-1">
                      {amenity}
                      <Button type="button" variant="ghost" size="sm" className="h-auto p-0 hover:bg-transparent" onClick={() => removeAmenityFromFacility(index, amenity)}>
                        <X className="h-3 w-3" />
                      </Button>
                    </Badge>)}
                </div>}
      ...
      <Button onClick={handleFinalSubmit} className="w-full h-12 text-lg font-semibold bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary/70" disabled={isLoading}>
        ...
        <Card className="w-full max-w-2xl shadow-2xl border-0 bg-card/80 backdrop-blur-sm">
          <CardHeader className="text-center space-y-4">
            ...
            <CardDescription className="text-lg">
              {currentStep === 1 ? "Creează contul și adaugă informațiile despre baza ta sportivă" : "Completează detaliile pentru fiecare facilitate"}
            </CardDescription>
          ...
          <div className="text-center pb-6">
            <p className="text-muted-foreground">
              Ai deja un cont?{" "}
              <Link to="/facility/login" className="text-primary hover:text-primary/80 font-semibold transition-colors">
                Conectează-te aici
              </Link>
            </p>
          </div>
